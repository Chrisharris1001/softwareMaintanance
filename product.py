...
#-----------------------------------------------------------------------------------------------------
    ...
    def fetch_cat_sup(self):
        ...
        try:
            cat_rows = fetch_query("SELECT name FROM category")
            if cat_rows:
                self.cat_list.clear()
                self.cat_list.append("Select")
                self.cat_list.extend([row[0] for row in cat_rows])

            sup_rows = fetch_query("SELECT name FROM supplier")
            if sup_rows:
                self.sup_list.clear()
                self.sup_list.append("Select")
                self.sup_list.extend([row[0] for row in sup_rows])
    ...
    def add(self):
        try:
            ...
            check_query = "SELECT * FROM product WHERE name=?"
            existing_product = fetch_query(check_query, (self.var_name.get(),))

            if existing_product:
                messagebox.showerror("Error", "Product already present", parent=self.root)
                return

            insert_query = """INSERT INTO product (Category, Supplier, name, price, qty, status) 
                              VALUES (?,?,?,?,?,?)"""
    ...
    def show(self):
        try:
            fetch_all_query = "SELECT * FROM product"
    ...
    def update(self):
        try:
            ...
            check_query = "SELECT * FROM product WHERE pid=?"
            existing_product = fetch_query(check_query, (self.var_pid.get(),))
    ...
    def delete(self):
        try:
            ...
            if op == True:
                delete_query = "DELETE FROM product WHERE pid=?"
                execute_query(delete_query, (self.var_pid.get(),))
    ...
    def search(self):
        try:
            ...
            search_text = f"%{self.var_searchtxt.get()}%"

            search_query = f"SELECT * FROM product WHERE {search_column} LIKE ?"
